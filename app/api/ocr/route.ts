...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // ocrmypdf 실행: 한국어+영어 OCR, 강제 OCR 적용
    const command = [
      'ocrmypdf',
      '-l kor+eng',        // 한국어 + 영어 인식
      '--force-ocr',       // 기존 텍스트 레이어가 있어도 강제 OCR
      '--optimize 1',      // PDF 최적화 (경량)
      '--clean',           // 이미지 정리 (노이즈 제거)
      '--deskew',          // 기울어진 페이지 자동 보정
      '--skip-big 100',    // 100메가픽셀 초과 이미지 건너뜀
      '--jobs 2',          // 병렬 처리
      `"${inputPath}"`,
      `"${outputPath}"`,
    ].join(' ')
    ...
    if (error instanceof Error) {
      // ocrmypdf 관련 오류 메시지 정리
      let errorMessage = error.message

      if (errorMessage.includes('No such file or directory')) {
        errorMessage = 'OCR 엔진이 설치되지 않았습니다. 서버 관리자에게 문의하세요.'
      } else if (errorMessage.includes('PriorOcrFoundError')) {
        errorMessage = '이 PDF에는 이미 텍스트 레이어가 존재합니다.'
      } else if (errorMessage.includes('timeout')) {
        errorMessage = 'OCR 처리 시간이 초과되었습니다. 더 작은 파일로 시도해주세요.'
      }
